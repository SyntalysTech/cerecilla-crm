...
import { deleteAllClientes } from "./actions";
...
export function DangerZone() {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await deleteAllClientes();
      if (res.error) {
        setResult({ type: "error", message: res.error });
      } else {
        setResult({ type: "success", message: `Se han eliminado ${res.count} clientes correctamente` });
        setShowConfirm(false);
        setConfirmText("");
      }
    } catch {
      setResult({ type: "error", message: "Error al eliminar los clientes" });
    } finally {
      setDeleting(false);
    }
  };
